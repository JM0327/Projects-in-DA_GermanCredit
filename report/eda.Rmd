...
Here, we could see the comparison between the number of good credit and not good credit among the observations.Besides, it is obvious that this data set has unbalanced structure.      
...
Then, we use the different graphs to see the distribution of each variables.    
...
In this part, we would like to draw the profilling of observations distinguished by different response. Thereby, it is possible to observe the performance / situation of observations subjects with good credit in each variable through the portrait.    
```{r}
cat_colnames <- Credit %>% select(-1, -3,-11,-14,-23,-27,-29, -32) %>% colnames()

plot_list=list()
for (i in cat_colnames){
  print(ggplot(data = Credit) +
  geom_mosaic(aes(x = product(i, RESPONSE), fill=RESPONSE)))
}



ggplot(data = Credit) +
  geom_mosaic(aes(x = product(CHK_ACCT, RESPONSE), fill=RESPONSE))
...
plot_correlation(Credit, type= 'c', cor_args = list( 'use' = 'complete.obs')) #only show numerical value
